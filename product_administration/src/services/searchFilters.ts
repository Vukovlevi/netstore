import type { Product, NetworkStoreResult } from "../types/Types";

const API_URL = "./api/search_product";
const NET_API_URL = `${import.meta.env.VITE_STORE_ADMIN_URL}/api/network-search`
...
const buildParams = (filters: SearchFilters): Record<string, string> => {
  const params: any = {};
...
export const searchService = {
  ...
  networkSearch: async (filters: SearchFilters): Promise<NetworkStoreResult[]> => {
    const params = buildParams(filters);

    const res = await fetch(NET_API_URL, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(params),
      credentials: 'include'
    ...
    if (!res.ok) {
      throw new Error("Hiba a hálózatos keresés során");
    }
    return res.json();
  },
};