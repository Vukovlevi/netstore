...
export const brandService = {
  ...
  create: async (name: string, is_own: boolean, is_temporary: boolean): Promise<ApiResponse> => {
    const res = await fetch(API_URL, {
      method: 'POST',
  ...
  update: async (id: number, name: string, is_own: boolean, is_temporary: boolean): Promise<ApiResponse> => {
    const res = await fetch(API_URL, {
        method: 'PUT',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
