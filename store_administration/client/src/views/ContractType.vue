...
const currentContractType: Ref<ContractType | null, ContractType | null> =
  ref(null);
const mode: Ref<"all" | "single", "all" | "single"> = ref("all");
const isModalOpen = ref(false);
let toDeleteContractTypeId = 0;
...
async function getContractTypes() {
  try {
    ...
    feedback.value = {
      type: "error",
      message:
        "Ismeretlen hiba miatt nem sikerült lekérni a szerződéstípusokat!",
    };
    console.error(err);
  }
}
...
async function deleteContractType(contractTypeId: Number) {
  ...
  try {
    ...
    if (resp.status == 204) {
      ...
      feedback.value = {
        type: "success",
        message: "Szerződéstípus törlése sikeres!",
      };
      toDeleteContractTypeId = 0;
      return;
    }
    ...
    feedback.value = {
      type: "error",
      message: "Szerződéstípus törlése sikertelen!",
    };
  } catch (err) {
    feedback.value = {
      type: "error",
      message: "Ismeretlen hiba miatt nem sikerült törölni a szerződéstípust!",
    };
    console.error(err);
  }
}
...
function search(searchValue: string) {
  ...
  filteredContractTypes.value = contractTypes.filter(
    (x) =>
      x.name.toLowerCase().includes(searchValue.toLowerCase()) ||
      x.weeklyHours.toString() == searchValue
  );
}
...
function confirm() {
  isModalOpen.value = false;
  deleteContractType(toDeleteContractTypeId);
}
</script>
...
<template>
  <Modal
    v-if="isModalOpen"
    title="Megerősítés"
    message="Biztosan törölni akarja a szerződéstípust?"
    confirm-text="Törlés"
    @cancel="cancel"
    @confirm="confirm"
  />
  <div class="mx-auto max-w-7xl mt-[5rem]">
    ...
    >
      <ContractTypeCard
        ...
        @delete="(contractTypeId: number) => {toDeleteContractTypeId = contractTypeId; isModalOpen = true}"
      />
    </div>
    <ContractTypeData
      v-else
